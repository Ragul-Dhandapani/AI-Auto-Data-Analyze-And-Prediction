...
- <b>Model Type</b>: {request.model_name.replace('_', ' ').title()}
- <b>Performance</b>: R² = {formatted_r2_score}
- <b>Target Variable</b>: `{request.target_column}`
- <b>Features</b>: {features_list}
- <b>Generated by</b>: PROMISE AI
- <b>Date</b>: {Path('/app/backend').stat().st_mtime}
...
- <b>model_code.py</b>: Core model class with preprocessing and training logic
- <b>train_full_dataset.py</b>: Script to train on your full TB-scale dataset
- <b>predict.py</b>: Script to make predictions on new data
- <b>{request.model_name}.pkl</b>: Pre-trained model (trained on sample data)
...
1. <b>Use Chunked Processing</b>:
   ```python
   train_on_full_dataset(data_path="large_data.csv", chunksize=50000)
   ```

2. <b>Use Parquet Format</b> (faster than CSV):
   ```python
   df.to_parquet("data.parquet")
   df = pd.read_parquet("data.parquet")
   ```

3. <b>Use Dask for Distributed Processing</b>:
   ```python
...
1. <b>API Endpoint</b> (Flask example):
   ```python
...
2. <b>Batch Prediction</b> (scheduled job):
   ```python
   import schedule
...
<b>Generated by</b>: PROMISE AI Platform  
<b>Target Variable</b>: `{}`  
<b>Features</b>: {}  
<b>Problem Type</b>: {}  
<b>Number of Models Exported</b>: {}
...
'''
    ...
    for idx, model_data in enumerate(sorted_models, 1):
        ...
        if idx == 1:
            recommendation = "✅ <b>BEST</b> - Highest accuracy"
        elif idx == 2:
...
- <b>{}</b>: {} (Best among all tested models)
- <b>Ranking</b>: #1 out of {} models
'''.format(best_model_name.replace('_', ' ').title(), metric, formatted_best_score, len(all_model_info))
    ...
    readme += '''
<b>Key Strengths for Your Use Case:</b>
...
'''.format(len(request.feature_columns), pattern_strength, importance_feature, len(all_model_info))
    ...
    for model_data in all_model_info:
        readme += "- <b>{}</b>: Use `{}_model.py`\n".format(model_data['name'], model_data['name'])
    
    readme += '''
<b>Recommended</b>: Start with `{}_model.py` (best performance)
...
<b>Solution</b>: Use chunked processing
```bash
python train_full_dataset.py --chunksize 50000
```

### Issue: ImportError for model package
<b>Solution</b>: Ensure all requirements are installed
```bash
pip install -r requirements.txt
```

### Issue: Predictions don't match expected values
<b>Solution</b>: Verify your new data has the same features and distribution as training data
```python
...
- <b>PROMISE AI Platform</b>: https://emergentagent.com
- <b>Scikit-learn</b>: https://scikit-learn.org/stable/
- <b>XGBoost</b>: https://xgboost.readthedocs.io/
- <b>LightGBM</b>: https://lightgbm.readthedocs.io/
